(cond-expand
  ((host js)
   ...
   (define-primitive
     ...
     "prim1(cmd => str2scm((()=>{const r=(require('child_process').spawnSync(`${scm2str(cmd)}`,{shell:true}));return r.status!==0 ? `Error: ${String(r.stdout)||String(r.stderr)}` : String(r.stdout);})())),")
