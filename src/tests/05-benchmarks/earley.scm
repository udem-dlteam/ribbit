...
(define (expt b n)
    (define (expt-iter b n acc)
        ...
        (expt-iter b n 1))(define repetitions 5)
...
(define (run-bench run ok?)
  (time (run-bench-aux run ok?)));;; EARLEY -- Earley's parser, written by Marc Feeley.
...
(run-bench
    (lambda () (test (make-list 8 'a)))
    (lambda (r) (equal? r 429)))
