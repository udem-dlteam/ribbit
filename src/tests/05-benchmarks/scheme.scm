...
(define (expt b n)
    (define (expt-iter b n acc)
        ...
        (expt-iter b n 1))(define repetitions 200)
...
(define (run-bench run ok?)
  (time (run-bench-aux run ok?)));;; SCHEME -- A Scheme interpreter evaluating a sort, written by Marc Feeley.
...
(run-bench
    (lambda () (scheme-eval (hide 1 CODE)))
    (lambda (r) (equal? r OUTPUT)))
