...
(define (expt b n)
    (define (expt-iter b n acc)
        ...
        (expt-iter b n 1))(define repetitions 50000)
...
(define (run-bench run ok?)
  (time (run-bench-aux run ok?)))(define (create-n n)
(do ((n n (- n 1))
     (a '() (cons '() a)))
    ((= n 0) a)))
...
(define input (create-n 100))

(run-bench
    (lambda () (recursive-div2 input))
    (lambda (result) (equal? (length result) 50)))
