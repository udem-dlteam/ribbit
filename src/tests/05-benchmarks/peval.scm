...
(define (expt b n)
    (define (expt-iter b n acc)
        ...
        (expt-iter b n 1))(define repetitions 2)
...
(define (run-bench run ok?)
  (time (run-bench-aux run ok?)));;; PEVAL -- A simple partial evaluator for Scheme, written by Marc Feeley.
        ...
        (letrec ((reverse
          (lambda (in result)
            (if (pair? in)
              ...
              result))))
          (reverse input '()))))
    (hide 20 '((a b c d e f g h i j k l m n o p q r s t u v w x y z)))))
  (lambda (result)
    (and (list? result)
        (= (length result) 10)
        (equal? (list-ref result 9) EXPECTED))))
