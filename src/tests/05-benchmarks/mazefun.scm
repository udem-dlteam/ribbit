...
(define (expt b n)
    (define (expt-iter b n acc)
        ...
        (expt-iter b n 1))(define repetitions 10)
...
(define (run-bench run ok?)
  (time (run-bench-aux run ok?)));;; MAZEFUN -- Constructs a maze in a purely functional way,
...
(run-bench
    (lambda () (make-maze (hide 11 11) (hide 11 11)))
